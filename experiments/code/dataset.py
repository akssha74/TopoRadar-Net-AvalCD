    ...
    def __getitem__(self, idx: int):
        ...
        if self.is_train:
            if torch.rand(1).item() > 0.5:
                ...
                topo = TF.hflip(topo)
                mask = TF.hflip(mask)
                # Executed checkpoint-era convention: reflect aspect sine only.
                # The orbital look-alignment raster remains scene-fixed.
                topo[2, :, :] = -topo[2, :, :]
            if torch.rand(1).item() > 0.5:
                pre = TF.vflip(pre)
                post = TF.vflip(post)
                ...
                topo = TF.vflip(topo)
                mask = TF.vflip(mask)
                # Executed checkpoint-era convention: reflect aspect cosine only.
                # The orbital look-alignment raster remains scene-fixed.
                topo[3, :, :] = -topo[3, :, :]
