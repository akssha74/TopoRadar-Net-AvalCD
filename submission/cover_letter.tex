...
\begin{enumerate}
    ...
    \item \textbf{Heuristic Geomorphic Slope Prior (Geo-Loss):} We audit rather than idealize the prior: its mask overlaps $18.47\%$ of training positives and lowers held-out positive recall relative to No-GeoLoss, despite a favorable aggregate Troms{\o} precision/F1 trade-off. We therefore present it as a documented precision--recall heuristic, not a physical-admissibility mechanism.
    ...
    \item \textbf{Calibrated Operational Proxy with Failure Boundaries:} Validation-only Platt calibration, connected components, and a frozen OSM major-road snapshot yield $83.0\%$ precision and $62.5\%$ recall for Pamir corridor alerts, with all cached TopoRadar scene runs below 5 s, but all seeds miss the single Troms{\o} corridor avalanche. This directly measures and limits operational readiness rather than asserting dispatch benefit.
\end{enumerate}
