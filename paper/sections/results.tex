...
To characterize cross-system transfer, all models are trained on European Alps (Livigno) and Greenland (Nuuk) events and evaluated without site-specific tuning on two held-out scenes: Troms{\o}, Norway, and Pish, Tajikistan. Table~\ref{tab:cross_system_performance} reports pixel-level metrics across three independent training seeds (42, 123, and 456; mean $\pm$ standard deviation).
...
Table~\ref{tab:cross_system_performance} shows region- and seed-dependent transfer. In Pamir, TopoRadar-Net records $50.18\% \pm 4.40\%$ F1, compared with Attention U-Net ($48.73\% \pm 2.74\%$), ResU-Net ($49.25\% \pm 3.39\%$), SiamUNet-conc ($47.34\% \pm 0.70\%$), and SiamUNet-diff ($37.11\% \pm 2.44\%$). On the pooled two-system benchmark it records $63.79\% \pm 1.88\%$, compared with $63.41\% \pm 1.03\%$, $63.13\% \pm 0.79\%$, $62.92\% \pm 1.42\%$, and $54.87\% \pm 1.03\%$, respectively. Across seeds, only the difference from SiamUNet-diff is significant; comparisons with the three competitive baselines are not.

On the Scandinavian Arctic test scene (Troms{\o}), TopoRadar-Net reaches $77.40\% \pm 1.86\%$ pixel F1 and $63.16\% \pm 2.45\%$ IoU, compared with ResU-Net ($76.60\% \pm 1.30\%$), SiamUNet-diff ($72.64\% \pm 0.39\%$), and SiamUNet-conc ($78.92\% \pm 1.07\%$). SiamUNet-conc then records $47.34\%$ in Pamir, a region-dependent drop compatible with sensitivity to the changed mountain regime; the two-region contrast does not identify overfitting to a single factor.
...
\begin{itemize}
    ...
    \item \textbf{Geo-Loss Trade-off and Construct Audit:} In Troms{\o}, removing Geo-Loss changes precision from $75.48\%$ to $72.50\%$ and F1 to $76.70\%$, with a Seed-42 paired-cluster difference of $+3.16\%$ ($p = 0.002$). However, the heuristic penalty mask contains 19,663 of 106,442 valid positive training pixels ($18.47\%$), 824 of 86,802 validation positives ($0.95\%$), and 492 of 56,548 held-out positives ($0.87\%$). Across three seeds, positive-pixel recall inside the held-out penalty stratum is lower for the full model than No-GeoLoss in Troms{\o} ($50.47\% \pm 3.18\%$ vs.\ $71.29\% \pm 6.39\%$) and Pamir ($1.08\% \pm 1.52\%$ vs.\ $6.45\% \pm 6.97\%$). The full model's penalty-stratum F1 is higher in Troms{\o} ($55.10\%$ vs.\ $50.34\%$) but lower in Pamir ($0.82\%$ vs.\ $2.77\%$). Thus Geo-Loss is a precision--recall heuristic with a documented positive-label conflict, not a physical-admissibility mechanism.
...
To evaluate model stability across the broader AvalCD distribution beyond the zero-shot test scenes, Table~\ref{tab:nuuk_validation} reports performance on the 11 April 2021 polar-maritime validation cohort in Southwest Greenland (used for checkpoint selection and threshold calibration; $n=129$ polygons, 81,595 debris pixels).

\begin{table*}[t]
\caption{Performance on the 11 April 2021 polar-maritime validation scene in Southwest Greenland (used for checkpoint selection and threshold calibration; $n=129$ polygons, 81,595 debris pixels) across three seeds (mean $\pm$ SD).}
