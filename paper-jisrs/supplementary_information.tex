...
The publication archive contains three seeds for TopoRadar-Net, the bottleneck self-attention U-Net, and No-GeoLoss (nine checkpoints). For provenance continuity, the self-attention checkpoint filenames retain the legacy prefix \texttt{Swin-UNet\_}, and stored results use \texttt{Swin-UNet}/\texttt{Attention U-Net}; these are historical labels only. The instantiated class is \texttt{AttentionUNetAval}, with four-head bottleneck self-attention and no shifted-window transformer blocks. The verification suite checks stored-summary arithmetic, parameter counts, valid-mask confusion closure, affine physical-area denominators, calibration bounds, and construct-audit values; raw-raster retraining is outside the package gate. Persistent identifying repository links are withheld in this review copy to preserve double blindness and are provided to the editorial office.
